...
Fixed Syntax Errors - Railway Ready
"""
...
class CallGenerator:
    ...
    def generate_morning_call(cls):
        try:
            ...
            if bn and bn["range"] > range_size * 2:
                symbol = "BANKNIFTY"
                mult = 100
                current = bn["current"]
                high = bn["high"]
            ...
            call = {
                ...
                "quality_score": 0,
                "human_phrase": random.choice(cls.PHRASES["high_confidence"])
            }
    ...
    def generate_afternoon_call(cls):
        try:
            ...
            call = {
                ...
                "entry_time": datetime.now(pytz.timezone(Config.TIMEZONE)).strftime('%H:%M'),
                "quality_score": 0,
                "human_phrase": random.choice(cls.PHRASES["medium_confidence"])
            }
...
class Formatter:
    ...
    def call_msg(cls, call, is_vip=False):
        ...
        msg = random.choice(intros) + "

"
        msg += vip_badge + "🎯 *NEW TRADING CALL*

"
        msg += f"📌 *Call ID:* `{call['id']}`
"
        msg += f"📊 *Symbol:* `{call['symbol']}`
"
        msg += f"📈 *Direction:* {direction}
"
        msg += f"🧠 *Strategy:* `{call['strategy']}`

"
        msg += f"💰 *ENTRY:* `{call['entry']}`
"
        msg += f"🛑 *STOP LOSS:* `{call['sl']}`
"
        msg += f"🎯 *TARGET 1:* `{call['target1']}`
"
        msg += f"🎯 *TARGET 2:* `{call['target2']}`
"
        msg += f"🎯 *TARGET 3:* `{call['target3']}`

"
        msg += f"📐 *RRR:* `{call['rrr']}`
"
        msg += f"⏱ *Timeframe:* `{call['timeframe']}`
"
        msg += f"🕐 *Time:* `{call['entry_time']}`
"
        msg += f"⭐ *Quality Score:* `{call.get('quality_score', 0)}/100`

"
        msg += f"🗣 *Jiga Bhai keh raha hai:*
...
"
        msg += "✅ *Plan:*
"
        msg += "   • Entry ke baad har 5 points pe update
"
        msg += "   • Target 1 → 50% book
"
        msg += "   • Target 2 → SL entry pe trail
"
        msg += "   • Target 3 → Full exit
"
        msg += "   • Risky lage toh exit kar dena!

"
        msg += "⚠️ *Risk Warning:*
"
        msg += "_SL strict follow karna bhai!_

"
        msg += "💎 *VIP Access:* @jiga_bhai_vip
"
        msg += "⚠️ *Disclaimer:* Educational purpose only.

"
        msg += f"#{call['symbol']} #TradingCall #JigaBhai"
        return msg
    ...
    def update_msg(cls, update):
        ...
        msg = headers.get(action, "📊 *UPDATE*") + "

"
        msg += f"📌 *Call:* `{update['call_id']}`
"
        msg += f"📊 *Symbol:* `{update['symbol']}` {update['type']}
"
        msg += f"💰 *Price:* `{update['price']}`
"
        msg += f"📈 *P&L:* `{update['points']:+.2f} points`
...
"
        msg += "💎 *VIP:* @jiga_bhai_vip

"
        msg += f"#{update['symbol']} #JigaBhai"
        return msg
...
"
        msg += "Bhai log, live market snapshot dekh lo!
...
"
        msg += "Market trend follow karo, against mat jao!

"
        msg += "💎 *VIP:* @jiga_bhai_vip
"
        msg += "#MarketUpdate #JigaBhai"
        return msg
...
"
        msg += "🌍 *Global:*
"
        msg += "   • Dow/Nasdaq: Mixed cues
"
        msg += "   • SGX Nifty: Flat to positive

"
        msg += "🎯 *Strategy:*
"
        msg += "   • 9:18 AM — Opening Range Breakout call
"
        msg += "   • 9-11 AM — Morning session trades
"
        msg += "   • 1-3 PM — Afternoon session trades
"
        msg += "   • Sirf high-quality setups!

"
        msg += "⚡ *Reminder:*
"
        msg += "   • SL strict follow karna
"
        msg += "   • Risk management #1 priority
"
        msg += "   • Emotion control mein rakhna

"
        msg += "💎 *VIP:* @jiga_bhai_vip

"
        msg += "#GoodMorning #Nifty #JigaBhai"
        return msg
...
class Handlers:
    ...
    async def start(update, context):
        ...
        msg = "🙏 *Welcome to JIGA BHAI GUJARATI TRADER!*

"
        msg += f"Namaste {user.first_name} bhai!

"
        msg += "🤖 *Main kya karta hu:*
"
        msg += "   ✅ Real-time NSE data (3-sec refresh)
"
        msg += "   ✅ Quality trades only (Score 60+)
"
        msg += "   ✅ 9-11 AM and 1-3 PM trades
"
        msg += "   ✅ Human-like analysis and updates
"
        msg += "   ✅ Smart risk management

"
        msg += "⏰ *Schedule:*
"
        msg += "   🌅 9:00 AM — Morning setup
"
        msg += "   🎯 9:18 AM — Call 1 (Compulsory)
"
        msg += "   🎯 10:30 AM — Call 2 (If setup)
"
        msg += "   🎯 1:30 PM — Call 3 (If setup)
"
        msg += "   🎯 2:30 PM — Call 4 (If setup)
"
        msg += "   🌙 3:30 PM — Closing
...
def setup_jobs(app):
    ...
    jq.run_daily(Jobs.morning_setup, time=time(hour=9, minute=0, tzinfo=tz))
    jq.run_daily(Jobs.first_call, time=time(hour=9, minute=18, tzinfo=tz))
    jq.run_daily(Jobs.afternoon_call, time=time(hour=13, minute=30, tzinfo=tz))
    jq.run_daily(Jobs.afternoon_call, time=time(hour=14, minute=30, tzinfo=tz))

    for h in [10, 11, 14, 15]:
        jq.run_daily(Jobs.market_update, time=time(hour=h, minute=0, tzinfo=tz))

    jq.run_repeating(Jobs.tracker, interval=300, first=10)
    jq.run_daily(Jobs.reset_daily, time=time(hour=0, minute=1, tzinfo=tz))
